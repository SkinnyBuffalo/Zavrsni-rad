...
public class StanariGodine implements Initializable{
    @Override
    ...
    @FXML
    private DatePicker datumUseljenjaPicker;
    @FXML
    private DatePicker datumIseljenjaPicker;
    @FXML
    ...
    @FXML
    public void handleGetSviStanariGodine(ActionEvent event) throws SQLException {
        LocalDate datumUseljenja = datumUseljenjaPicker.getValue();
        LocalDate datumIseljenja = datumIseljenjaPicker.getValue();

        DateTimeFormatter formatter= DateTimeFormatter.ofPattern("yyyy-MM-dd");

        String formattedDatumUseljenja = datumUseljenja.format(formatter);
        String formattedDatumIseljenja = datumIseljenja.format(formatter);

        getSviStanariGodine(formattedDatumUseljenja,formattedDatumIseljenja);
    }


    public void getSviStanariGodine(String formattedDatumUseljenja, String formattedDatumIseljenja) throws SQLException {
        Connect connect=new Connect();
        Connection connection=connect.connect();
        PopupAlert popupAlert=new PopupAlert();

        String upit = "SELECT * FROM `stanar` RIGHT JOIN `boravak` ON `boravak`.`oib` = `stanar`.`oib` WHERE `boravak`.`datum_useljenja` >= '" + formattedDatumUseljenja + "' AND `boravak`.`datum_iseljenja` <= '" + formattedDatumIseljenja + "'";
        PreparedStatement preparedStatement=connection.prepareStatement(upit);
        ResultSet resultSet=preparedStatement.executeQuery();

        if(!resultSet.next()) {
            popupAlert.showAlert(Alert.AlertType.ERROR, "Error", "Ne postoji stanar koji je boravio u tom vremenskom periodu, ili je ne ispravan vremenski period");
        }
        else {
